...
- **Push (see `supabase/push-setup.sql`):** `push_tokens` table (+RLS `push_tokens_own`), `pg_net` enabled, and two `AFTER INSERT` triggers — `notify_push` on `notifications` and `message_push` on `messages` — both call the `send-push` edge fn via `net.http_post` with the `PUSH_HOOK_SECRET` (set as an edge-function secret). **⚠️ Since migration 0038 (10 Sep 2026) the trigger bodies do NOT hold the literal** — both functions read it from Vault as `push_hook_secret`. Never paste it into a function body again: that is how it ended up in `pg_proc` (audit item 36). Rotating means setting the same new value in Vault and in the edge-function secret.
