...
### 🟢 PRE-SUBMISSION COMPLIANCE AUDIT (2026 Play + App Store policies) — ALL ITEMS CLOSED
Audited 13 items against current store policy. 7 were already compliant (privacy/terms links in-app, active terms acceptance at signup, email/password only so **Sign in with Apple is NOT required**, Android target API 36, iOS `PrivacyInfo.xcprivacy` auto-generated by SDK 56, permissions all mapped to real features, friendly empty states). The gaps, all now fixed:
- **Age gate → real DOB (commit `44faa72`).** Was an 18+ tick-box only. Signup now takes DD/MM/YYYY (pure JS, no native picker) and blocks: missing parts, impossible dates (31/02 — built from parts and round-trip checked, since JS `Date` silently rolls those over), future dates, >120 years, and under-18. Stored as `date_of_birth` (ISO) in auth metadata; `ensureProfile` backfills `public.users.date_of_birth` when null (the signup trigger creates the row without it). Admin Users has an Age column. SQL: `supabase/dob-age-gate.sql`.
- **Reviewer access to the paid MODEL tier (commit `12b210e`).** App Review can't pay real money now Stripe is live, and only the provider side had a comp path. Added `users.subscription_waived` mirroring `provider_fee_waived` — `hasActiveSubscription()` honours it, admin Users has a **"Free sub"** toggle + purple **Comp** badge, Settings shows "✨ Complimentary" with no Upgrade/Cancel. Doubles as the comp/promo lever. SQL: `supabase/subscription-waive.sql`.
- **In-app support route (commit `e768a30`).** `support@guineapigapp.co.uk` was plain text inside error alerts only; now a **Support → Contact support** row in Settings opening `mailto:` (with a fallback alert if no mail app). Pairs with the existing in-chat Report flow for Google's feedback requirement.
- **Subscription terms out of the fine print (commit `e768a30`).** Price, renewal frequency, auto-renewal and cancellation now all stated adjacent to the CTA (Apple 3.1.2), not in an 11px footer.
- **Refund/cancellation copy (commit `c6d3910`).** Both paid screens now disclose the policy before the charge, with **Terms** linking to `guineapigapp.co.uk/terms` (§9).
- **⚠️ CSAE clause + Play Console child-safety declaration — ✅ DONE by Micky (web/console).** Google requires apps with social features to explicitly prohibit child sexual abuse and exploitation in publicly accessible standards. Terms updated + Play Console declaration completed. **This was the last hard launch blocker.**

### 🟢 UI/UX PRE-SUBMISSION POLISH (commit `8ace444`) — fixed states a reviewer would read as broken
Design review of the 7 priority screens; shipped the high-impact/low-risk set. The serious ones were behavioural, not cosmetic:
- **Stylist dashboard span "Finding models near you…" FOREVER when location was denied** (coords never arrive, so the guard kept re-arming the spinner). Reviewers routinely deny permissions. Now explains itself and stops; the GPS-failure paths had the same bug and are covered too.
- **Model home silently discarded ALL filters when nothing matched** and re-rendered the full list with the chips still lit — search looked broken/dishonest. Now a real "No matches" state with **Clear filters**, distinct from the genuine "No stylists yet".
- **26 of 28 stylist filter chips always returned zero** because `nearby_models` didn't select the hair/skin columns — the data existed in `model_attributes` all along. RPC now `LEFT JOIN`s it (SQL: `supabase/nearby-models-attributes.sql`; **LEFT is deliberate** — an inner join would drop every model who hasn't filled attributes). Filters work and the model cards regained their attribute chips.
- Raw Postgres error (code/message/details/hint) was alerted to stylists on first load → human copy, detail to `console.error`.
- **New accepted booking opened to a blank message area** → chat empty state. Rendered as a **sibling, not `ListEmptyComponent`** — `inverted` flips the list's whole subtree, which renders anything inside it mirrored.
- Booking times showed raw `09:00:00 – 11:00:00` → `9:00am` (same formatter as `sessions.tsx`).
- Misleading copy fixed: "Save **models** you love" on the model's own favourites (they're stylists); "All applications reviewed" greeting stylists who never had one; verify-payment's "Back to settings" that called `router.back()`; completed chat's composer vanishing with no explanation; subscribe not disclosing the identity check before charging.
- Signup cooldown started **before** the request and wasn't reset on failure, so a typo locked the button for 60s.

### 🟢 APP ICON + SPLASH (commit `71f6805`)
Wordmark dropped (the launcher mask was clipping it to "uinea Pig Ap"); guinea pig centred on brand pink `#FFE3EF`, 1024×1024, adaptive foreground at 60% inside the ~66% safe zone. **Also caught two Expo-template leftovers that would have shipped:** `ios.icon` pointed at the stock `assets/expo.icon` (blue gradient + **Expo logo**), which *overrides* the top-level icon on iOS — removed; and the splash background was `#208AEF` bright blue — now pink.
⚠️ **Icons/splash are native config — they need a NEW BUILD, not a Metro reload.**
...
- **Native/ops (one-time, all done):** required a fresh **EAS dev build** (native module — a Metro reload can't add it); `expo-notifications` plugin + `android.googleServicesFile` wired in `app.json`; **FCM** set up (Android `google-services.json` present locally + **gitignored**; FCM V1 service-account key uploaded to Expo); `PUSH_HOOK_SECRET` set as an edge secret; `supabase/push-setup.sql` run in the SQL editor (table + RLS + `pg_net` + both triggers). **⚠️ Superseded for the secret by migration 0038 (10 Sep 2026):** the instruction here was to re-paste the secret into the trigger bodies, which put it in `pg_proc` (audit item 36). Both functions now read it from Vault as `push_hook_secret`; never paste it into a function body.
...
- ✅ **DONE + VERIFIED (17 Jul) — Stripe switched to LIVE; a real live £4.99 subscription succeeded.** `pk_live` swapped in mobile `.env` + `eas.json` (preview **and** production — production previously had NO env block); edge fns `stripe-payment` + `delete-account` redeployed on `sk_live` + live `STRIPE_MONTHLY_PRICE_ID` (`price_1Tu7cA…`). **STANDARD Stripe (Payment Intents + Subscriptions, we're merchant of record) — NOT "Managed Payments".** VAT/tax is now ours. `sk_live` is only in Supabase secrets, never in the repo. **Debug note:** first attempts 500'd with `Invalid API Key` because the `sk_live` was pasted with a stray trailing char; the tell was that Stripe's own account logs showed NOTHING (unauthenticated requests aren't attributed) — the real error only appeared in the Supabase edge-function logs. Re-setting the key cleanly fixed it. (Stale lowercase `stripe_secret_key` secret still exists — harmless, delete later.)
