...
- **Admin access = a row in the `admins` table** (keyed to the immutable `auth.users.id`); enforced server-side by `proxy.ts` (`is_admin()` on every route) + RLS. **Two admins:** a **dedicated console admin, `admin@guineapigapp.co.uk`, `8788ed3d…`**, meant as the primary, plus **Micky B `ff06d568…`**, meant as break-glass backup. **⚠️ Both halves of that are false in practice (read from the database 10 Sep 2026 — audit items 34 and 35):** (1) this file said the dedicated admin is *never an app user*; it has a `public.users` row with `role = 'model'`, and how it got one is not established. (2) It made **2 of 30** logged verification decisions, all on 26–27 Jul, last sign-in 26 Jul. **Micky B made 28** — break-glass became the normal path by drift, and that account also holds a provider profile in the product it moderates. Intent unchanged: never sign the dedicated admin into the mobile app; never delete its auth user (`admins.user_id` cascades). Do not rely on "the console admin has no app account" in any code or migration — `0036` did, and was right about the fix for the wrong reason.
